...
public class APIrequests {
    ...
    private void average_power_per_day(final Context context)               //////   for the current day
    {
        ...
        Call<SummaryOfDay> summaryOfDay = SolarApi.getService(context).getSummaryOfToday("system id","current date","api key","user id");
    ...
    {
        Call<WeeklyValues> weeklyValues = SolarApi.getService(context).getValuesofWeek("system id","start date","end date","api key","uer id");
    ...
    {
        Call<PostData> postData = SolarApi.getService(context).getPostData("apikey","user id");
